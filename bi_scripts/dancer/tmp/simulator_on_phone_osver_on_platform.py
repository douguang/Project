...
# -*- coding: UTF-8 -*-
'''
@Time    : 16-12-21 下午8:21
@Author  : Andy 
@File    : simulator_on_phone_osver_on_platform.py
@Software: PyCharm
Description :  武娘模拟器用户 分渠道 游戏版本 手机型号的转换率
'''
...
def data_reader(file_url):
    ...
    osver_phone_no_df = osver_phone_no_df.groupby(['device','plat', 'time', 'osver','phone',]).agg(
        {'ip': lambda g: g.nunique()}).reset_index()
    ...
    osver_phone_you_df = osver_phone_you_df.groupby(['device', 'plat','time', 'osver','phone',]).agg(
        {'ip': lambda g: g.nunique()}).reset_index()
    ...
    result_df = pd.DataFrame(osver_phone_no_df).merge(osver_phone_you_df,on=['device', 'plat','time', 'osver','phone',],how='left')
    ...
    result_df.to_excel("/home/kaiqigu/桌面/武娘_模拟器用户_分渠道-小米ee.xlsx", index=False)
