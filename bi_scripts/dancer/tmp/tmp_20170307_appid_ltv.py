...
def dis_reg_user_ltv(start_date, date):
    ...
    # 注册数据
    reg_sql = '''
    select t1.ds, t1.account, t2.appid from (
    ...
    for date_to_update in dates_to_update:
        ...
        if date_to_update >= '20161110':
            ...
            ltv_dfs = use_reg_df.groupby(['ds', 'appid']).agg({'account': lambda g: g.nunique()}).reset_index().rename(
                columns={
            ...
            for ltv_ds in ltv_days:
                ...
                if ltv_end_date <= date:
                    ...
                    result_ltv_df = use_reg_df.merge(
                        use_pay_df, on='account', how='left').fillna(0)
                    ...
                    ltv_dfs = ltv_dfs.merge(
                        result_ltv_df, on=['ds', 'appid', 'reg_user_num'])
            ...
            columns = sum((['d%d_pay_num' % ltv_day, 'd%d_ltv' % ltv_day, ] for ltv_day in ltv_days), ['ds', 'reg_user_num', 'appid'])
...
if __name__ == '__main__':
    for platform in ['dancer_pub']:
        ...
        result = dis_reg_user_ltv('20170329', '20170404')
