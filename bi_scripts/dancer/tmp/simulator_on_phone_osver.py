...
# -*- coding: UTF-8 -*-
'''
@Time    : 16-12-20 下午4:44
@Author  : Andy 
@File    : simulator_on_phone_osver.py
@Software: PyCharm
Description :
'''
...
def data_reader(file_url):
    ...
    result_noaccount_df =result_noaccount_df.groupby(['device','time',]).agg(
    ...
    result_account_df = result_account_df.groupby(['device', 'time', ]).agg(
        {'ip': lambda g: g.nunique()}).reset_index()
    ...
    osver_phone_no_df = osver_phone_no_df.groupby(['device', 'time', 'osver','phone',]).agg(
        {'ip': lambda g: g.nunique()}).reset_index()
    ...
    osver_phone_you_df = osver_phone_you_df.groupby(['device', 'time', 'osver','phone',]).agg(
        {'ip': lambda g: g.nunique()}).reset_index()
    ...
    result_df = pd.DataFrame(osver_phone_no_df).merge(osver_phone_you_df,on=['device', 'time', 'osver','phone',],how='left')
    result_df = pd.DataFrame(result_df).merge(result_noaccount_df,on=['device', 'time',],how='left')
    result_df = pd.DataFrame(result_df).merge(result_account_df,on=['device', 'time',],how='left')
    result_df = pd.DataFrame(result_df).fillna(0)
    result_df.to_excel("/home/kaiqigu/桌面/武娘_模拟器用户.xlsx", index=False)
