...
'''
Author      : Hu Chunlong
Description : 补武娘台湾11月18号ltv，先不删除
'''
...
def dis_reg_user_ltv(start_date, date):
    ...
    where ds = '20161118' and account not in (
        select account from mid_info_all where ds='20161117'
    )
    group by account,ds
    ...
    result_df = result_df[result_df.ds == '20161118']
    # return result_df

    # 更新MySQL表——每日LTV
    table = 'dis_reg_user_ltv'
    del_sql = 'delete from {0} where ds = {1}'.format(table, '20161118')
...
if __name__ == '__main__':
    for platform in ['dancer_tw']:
        ...
        result = dis_reg_user_ltv('20161115','20161207')
        # result.to_excel('/home/kaiqigu/Documents/dancer_ltv.xlsx')
