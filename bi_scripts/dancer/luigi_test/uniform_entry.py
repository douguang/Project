    ...
    def workflow(self):
        ...
        if self.task_name.startswith('raw_'):
            ...
            tasks.append(self.new_task('MidHqlEntry',
                                       ...
                                       task_name=self.task_name,
                                       date=self.date))
        elif self.task_name.startswith('mid_info_all'):
            tasks.append(self.new_task('MidHqlEntry',
                                       MidHqlEntry,
                                       platform=self.platform,
                                       task_name=self.task_name,
                                       date=self.date))
        elif self.task_name.startswith('raw_paylog'):
            tasks.append(self.new_task('RawEntry',
                                       RawEntry,
                                       platform=self.platform,
                                       task_name=self.task_name,
...
class DancerDaily(sl.WorkflowTask):
    '''
    执行入口
    '''
    date = luigi.DateParameter(
...
class DancerDailyRaw(sl.WorkflowTask):
    '''
    执行入口
    '''
    date = luigi.DateParameter(
...
class DancerDailyPaylog(sl.WorkflowTask):
    '''
    执行入口
    '''
    date = luigi.DateParameter(
    ...
    def workflow(self):
        ...
        # 
        for job_deps in settings_dev.job_deps:
            if job_deps.startswith('raw_paylog'):
                tasks.append(self.new_task('UniformEntry',
                                           UniformEntry,
                                           platform=self.platform,
                                           task_name=job_deps,
...
# 临时用，之后补Mart数据
class DancerDailyMid(sl.WorkflowTask):
    '''
    执行入口
    '''
    date = luigi.DateParameter(
...
# 临时用，之后补Mart数据
class DancerDailyMidInfoAll(sl.WorkflowTask):
    '''
    执行入口
    '''
    date = luigi.DateParameter(
    ...
    def workflow(self):
        ...
        # 跑所有表
        # for job_deps in settings_dev.job_deps:
        if job_deps.startswith('mid_info_all'):
            tasks.append(self.new_task('UniformEntry',
                                       UniformEntry,
                                       platform=self.platform,
...
class DancerDailyMart(sl.WorkflowTask):
    '''
    执行入口
    '''
    date = luigi.DateParameter(
