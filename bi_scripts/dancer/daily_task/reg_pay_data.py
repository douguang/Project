...
@author: Andy 
@site:  分包新增及收入
@software: PyCharm  ff:ff:ff:ff:ff:ff
@file: reg_pay_data.py 
@time: 18/1/30 下午3:18 
"""
...
def data_reduce():
    ...
    # info_cgame_df.to_excel(r'/Users/kaiqigu/Documents/Dancer/武娘-cgame-deivcemark重合数据-info_cgame_df_20180130.xlsx', index=False)
    info_cgame_df = info_cgame_df[['device_mark','account','reg_time']]
    print info_cgame_df.head()

    settings_dev.set_env('dancer_pub')
    info_pub_sql = '''
        select device_mark,account as account_pub from mid_info_all where ds='20180129' and  act_time >='2017-10-29 00:00:00' and   device_mark != '' and   device_mark != 'ff:ff:ff:ff:ff:ff' and   device_mark != '02:00:00:00:00:00' and  device_mark != '00:00:00:00:00:00' and device_mark != '00000000-0000-0000-0000-000000000000' group by device_mark,account_pub
        '''
    #    where t1.device_mark != '00000000-0000-0000-0000-000000000000'
    print info_pub_sql
    info_pub_df = hql_to_df(info_pub_sql)
    print info_pub_df.head()
    # info_pub_df.to_excel(r'/Users/kaiqigu/Documents/Dancer/武娘-cgame-deivcemark重合数据-info_pub_df_20180130.xlsx', index=False)


    settings_dev.set_env('dancer_bt')
    info_bt_sql = '''
            select device_mark,account as account_bt from mid_info_all where ds='20180129' and  act_time >='2017-12-05 00:00:00' and   device_mark != '' and   device_mark != 'ff:ff:ff:ff:ff:ff' and  device_mark != '02:00:00:00:00:00' and  device_mark != '00:00:00:00:00:00' and device_mark != '00000000-0000-0000-0000-000000000000' group by device_mark,account_bt
            '''
    #    where t1.device_mark != '00000000-0000-0000-0000-000000000000'
    print info_bt_sql
    info_bt_df = hql_to_df(info_bt_sql)
    print info_bt_df.head()
    # info_bt_df.to_excel(r'/Users/kaiqigu/Documents/Dancer/武娘-cgame-deivcemark重合数据-info_bt_df_20180130.xlsx', index=False)


    res_df = info_cgame_df.merge(info_pub_df, on=['device_mark', ], how='left')
    res_df = res_df.merge(info_bt_df, on=['device_mark', ], how='left')
    print res_df.head()
    res_df.to_excel(r'/Users/kaiqigu/Documents/Dancer/武娘-cgame-deivcemark重合数据-1_20180130.xlsx', index=False)
    ...
    result_df.to_excel(r'/Users/kaiqigu/Documents/Dancer/武娘-cgame-deivcemark重合数据-2_20180130.xlsx', index=False)
