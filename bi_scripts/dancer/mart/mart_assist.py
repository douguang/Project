...
import time
import numpy as np
from mart_sql_cfg import dancer_parse_actionlog_all_sql, dancer_raw_info_sql, dancer_nginx_sql, paylog_sql, spendlog_sql, dancer_parse_actionlog_all_2_sql


def mart_assist(date):
    print time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))

    df_1, df_2 = hqls_to_dfs([dancer_raw_info_sql.format(date=date), dancer_parse_actionlog_all_sql.format(
        date=date)])
    df_2 = df_2.drop_duplicates(['user_id'])
    df_4, df_5, df_6 = hqls_to_dfs([paylog_sql.format(date=date), spendlog_sql.format(
        date=date), dancer_parse_actionlog_all_2_sql.format(date=date)])

    ori_df = df_1.merge(df_2, on='user_id', how='left').merge(df_4, on='user_id', how='left').merge(
        df_5, on='user_id', how='left').merge(df_6, on='user_id', how='left')
