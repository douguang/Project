...
def dis_country_ltv(date):
    ...
    # 注册数据
    reg_ip_sql = '''
    SELECT to_date(reg_time) AS reg_time,
           account,
           regist_ip
    FROM mid_info_all
    WHERE ds = '{date}'
    ...
    for date_to_update in dates_to_update:
        ...
        if date_to_update >= server_start_date:
            ...
            ltv_dfs = use_reg_df.groupby(['ds', 'country']).agg(
            ...
            for ltv_ds in ltv_days:
                ...
                if ltv_end_date <= date:
                    ...
                    result_ltv_df['d%s_ltv' % ltv_ds] = result_ltv_df[
                        'd%s_pay_rmb' % ltv_ds] / result_ltv_df['reg_user_num']
                    ltv_dfs = ltv_dfs.merge(
                        result_ltv_df,
                        on=['ds', 'country', 'reg_user_num'])
                else:
