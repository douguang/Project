...
def dis_activity_unique_shop(date):
    version, act_start_time, act_end_time = get_active_conf('unique_shop',
                                                            date)
    if version != '':
...
def dis_unique_shop(act_start_short,act_end_short,act_start_time,act_end_time):
    sql = '''
    SELECT user_id,reverse(substr(reverse(user_id), 8)) as server,a_tar,return_code,vip
