...
def dis_daily_data(date):
    ...
    new_order_df, new_df, pay_df, info_df, action_df = hqls_to_dfs(
        [new_order_sql, new_sql, pay_sql, info_sql, action_sql])
    ...
    new_df = new_df.merge(day_action_df,
                          on=['ds', 'user_id'],
                          how='left').fillna('None')
    pay_df = pay_df.merge(day_action_df,
                          on='user_id', how='left').fillna('None')
    ...
    # 充值总额
    pay_df_result = pay_df.groupby(['ds', 'platform']).agg({
        'user_id': 'count',
        'pay_money': 'sum'
...
if __name__ == '__main__':
    for platform in ['dancer_tw']:
        ...
        for date in date_range('20170523', '20170613'):
            print platform, date
            dis_daily_data(date)
