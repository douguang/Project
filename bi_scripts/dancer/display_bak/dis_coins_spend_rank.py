...
def dis_coins_spend_rank(date):
    ...
    FROM
      (SELECT user_id,
              ...
              Row_Number() OVER (ORDER BY sum(coin_num) DESC) rank
       FROM raw_spendlog
    ...
    LEFT OUTER JOIN
      ( SELECT user_id,
        ...
        FROM raw_paylog
        WHERE ds <= '{date}'
          AND ds >= '{server_start_date}' and platform_2<>'admin_test' AND order_id not like '%testktwwn%'
    ...
    WHERE rank <= 100
    ORDER BY rank
    '''.format(date=date, server_start_date=server_start_date, zichong_uids=zichong_uids)
    df = hql_to_df(sql)
    df = df.fillna(0)
    # print df
    #更新MySQL
