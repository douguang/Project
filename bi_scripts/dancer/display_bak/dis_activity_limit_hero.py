...
def dis_activity_limit_hero_one(act_start_short,act_end_short,act_start_time,act_end_time):
    ...
    #action信息
    active_sql = '''
         select user_id, a_tar from parse_actionlog where ds>='{act_start_short}' and ds<='{act_end_short}' and log_t>='{act_start_time}' and log_t<='{act_end_time}'
    ...
    spend_sql = '''
        ...
        and goods_type = 'gacha.get_gacha' group by user_id
    '''.format(act_start_short=act_start_short, act_end_short=act_end_short, act_start_time=act_start_time, act_end_time=act_end_time)
    # print spend_sql
