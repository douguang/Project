...
def dis_activity_contract_one(act_start_short,act_end_short,act_start_time,act_end_time):
    ...
    contract_sql = '''
        select
          user_id, (NVL(freemoney_diff,0) + NVL(money_diff,0)) as spend, server, a_tar
        from
          parse_actionlog
        where
          ds>='{act_start_short}' and ds<='{act_end_short}' and log_t>='{act_start_time}' and log_t<='{act_end_time}' and a_typ = 'magic_school.open_contract'
    ...
    info_sql = '''
        select user_id, vip from parse_info where ds='{date}'
    '''.format(date=act_start_short)
    info_df = hql_to_df(info_sql)
    # print info_df.head(10)

    pay_sql = '''
        select user_id, sum(order_money) as money from raw_paylog where ds='{date}' and platform_2<>'admin_test' and order_id not like '%testktwwn%' group by user_id
    '''.format(date=act_start_short)
