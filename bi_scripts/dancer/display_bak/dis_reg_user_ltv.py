...
def dis_reg_user_ltv(date):
    ...
    # 注册数据
    reg_sql = '''
        select regexp_replace(to_date(reg_time),'-','') as ds, account from (
            select reg_time, account, row_number() over(partition by account order by reg_time) as rn
        from mid_info_all
        where ds = '{date}' and to_date(reg_time) <= '{f_date}') t1 where t1.rn=1
            ...
            for ltv_day in ltv_days:
                ...
                if ltv_end_date > date:
                    ...
                    user_pay_df = pay_df[(pay_df.ds >= date_to_update) & (
                        pay_df.ds <= ltv_end_date) & (pay_df.account.isin(date_reg_accounts))]
