...
def dis_country_ltv(date):
    ...
    # 注册数据
    reg_ip_sql = '''
    select to_date(reg_time) as reg_time, account, regist_ip
    from mid_info_all
    where ds = '{date}'
      and to_date(reg_time) >= '2016-11-25' and to_date(reg_time) <= '{f_date}'
        ...
        if date_to_update >= server_start_date:
            ...
            for ltv_ds in ltv_days:
                ...
                if ltv_end_date <= date:
                    ...
                    result_ltv_df['d%s_ltv' % ltv_ds] = result_ltv_df['d%s_pay_rmb' % ltv_ds] / result_ltv_df[
                        'reg_user_num']
                    ltv_dfs = ltv_dfs.merge(result_ltv_df, on=['ds', 'country', 'reg_user_num'])
                else:
