...
def dis_new_big_r_info(date):
    ...
    SELECT
        ...
        level,
        vip,
        coalesce(spend_coin_sum, 0) as spend_coin_sum,    /*nan补0*/
        pay_rmb_sum,
        charge_coin_sum,
        first_pay_date,
        last_pay_date,
    ...
    FROM
        ...
        SELECT
            ...
            sum(gift_coin)+sum(order_coin) AS charge_coin_sum,        /*充值钻石+赠送钻石*/
            min(ds) AS first_pay_date,
            max(ds) AS last_pay_date,
            sum(CASE WHEN ds < '{date}' THEN order_money ELSE 0 END) AS pay_rmb_sum_yestoday,    /*截至昨天的充值金额 CASE WHEN THEN ELSE*/
            count(DISTINCT CASE WHEN ds >= '{date_before_seven}' AND ds <= '{date}' THEN ds ELSE NULL END) AS d7_pay_date_num  /*统计不同的充值日期作为充值天数*/
        FROM
            raw_paylog
        WHERE
            ds <= '{date}' and platform_2<>'admin_test' AND order_id not like '%testktwwn%'
        GROUP BY
            user_id
        HAVING
            pay_rmb_sum_yestoday < {pay_standard} AND pay_rmb_sum > {pay_standard}   /*通过昨天与今天的累计充值情况以及设定的充值标准来确定鲸鱼*/
        ) t1
    LEFT JOIN
        (
        SELECT
