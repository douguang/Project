...
def dis_ip_country_ltv(date):
    ...
    # 注册数据
    reg_ip_sql = '''
    select regexp_replace(to_date(reg_time),'-','') as reg_time, account, regist_ip from (
        select reg_time, account, regist_ip, row_number() over(partition by account order by reg_time) as rn
    from mid_info_all
    where ds = '{date}'
      and to_date(reg_time) >= '2016-11-25' and to_date(reg_time) <= '{f_date}') t1 where t1.rn=1
