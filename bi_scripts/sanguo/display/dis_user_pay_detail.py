...
def dis_user_pay_detail(date):
    ...
    # account 用户数据
    account_df['num'] = 1
    account_data = account_df.groupby('user_id').sum().num.reset_index()
    # 滚服的account
    ...
    reg_user_num = day_reg_user_df.user_id.nunique()
    dau = info_df.user_id.nunique()
    ...
    # 新增付费人数、金额
    day_pay_df['is_ago'] = day_pay_df['user_id'].isin(ago_pay_df.user_id.values)        #新增一列'is_ago'，用来记录今天付费用户之前是否付费，若付费则取1否在取0
    new_pay_df = day_pay_df[~day_pay_df['is_ago']]                                      #将'is_ago'取反，用来记录是否为新付费用户
    new_pay_df['is_old'] = new_pay_df['account'].isin(gun_account_df.user_id.values)
    new_pay_df = new_pay_df[~new_pay_df['is_old']]
    new_pay_num = new_pay_df.user_id.nunique()
    ...
    # 当日新增付费人数、金额
    day_pay_df['is_reg'] = day_pay_df['user_id'].isin(day_reg_user_df.user_id.values)   #判断付费用户是否为当日注册
    new_day_pay_df = day_pay_df[day_pay_df['is_reg']]
    new_day_pay_df['is_old'] = new_day_pay_df['user_id'].isin(gun_account_df.user_id.values)
...
if __name__ == '__main__':
    #for platform in ['sanguo_ks','sanguo_tw','sanguo_kr','sanguo_tx','sanguo_in','sanguo_tl']:
    for platform in ['sanguo_tt',]:
          settings_dev.set_env(platform)
          for date in date_range('20170407','20170423'):
