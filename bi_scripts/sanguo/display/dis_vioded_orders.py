...
def dis_vioded_orders(date):
    ...
    print date,table
    vioid_data_sql = '''
        select t1.ds,t1.purchasetimemillis,t1.voidedtimemillis,t2.developerPayload,t2.orderid,t2.productid,t2.ip from (
        select ds,purchasetimemillis, voidedtimemillis, purchasetoken from parse_voided_data where ds='{date}' group by ds,purchasetimemillis, voidedtimemillis, purchasetoken
        )t1 left outer join(
        select purchasetoken,developerPayload, orderid,  productid, ip from parse_sdk_nginx where ds>='{date_in_3days}' and ds<='{date}' group by purchasetoken, developerPayload, orderid,  productid, ip
        )t2  on t1.purchasetoken=t2.purchasetoken
        group by t1.ds,t1.purchasetimemillis,t1.voidedtimemillis,t2.developerPayload,t2.orderid,t2.productid,t2.ip
    '''.format(**{
        'date': date,
        'date_in_3days': ds_add(date, -5),
    })
    ...
    if vioid_data_df.__len__() != 0:
        ...
        # print vioid_data_df
        vioid_data_df = vioid_data_df[['ds','order_ds','order_id','product_id','user_id','voided_ds','ip','country',]]
        print vioid_data_df
        # 更新MySQL表
...
if __name__ == '__main__':
    ...
    for platform in ['sanguo_tl',]:
        ...
        for date in date_range('20170923', '20170924'):
