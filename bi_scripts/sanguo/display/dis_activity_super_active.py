...
Description : 宇宙最强活动展示页面,取全服玩家的id vip 服务器 战斗力 活动消费 当日充值，按照消费降序排名，计算服务器消费总量。(已完成)
Database    : dancer
'''
import settings_dev
...
def dis_activity_super_active_one(act_start_short,act_end_short,act_start_time,act_end_time):

    #用户信息
    info_sql = '''
        select ds, user_id, reverse(substr(reverse(user_id),8)) as server, vip from raw_info where ds >= '{act_start_short}' and ds<='{act_end_short}'
    ...
    spend_sql = '''
        ...
        group by user_id order by spend desc limit 500
    ...
    #充值信息
    pay_sql = '''
        select user_id, sum(order_money) as money from raw_paylog where ds >= '{act_start_short}' and ds<='{act_end_short}' group by user_id
    '''.format(act_start_short=act_start_short, act_end_short=act_end_short)
    # print pay_sql
    ...
    columns = ['ds', 'user_id', 'server', 'vip', 'money',  'spend', 'server_spend']
...
if __name__ == '__main__':
    ...
    for platform in ('sanguo_tw', 'sanguo_ks', 'sanguo_kr', 'sanguo_ios', 'sanguo_tx', 'sanguo_in', 'sanguo_tl'):
        settings_dev.set_env(platform)
        for date in date_range('20170101', '20170204'):
