...
def dis_keep_rate(excute_date):
    ...
    for reg_date in reg_dates:
        ...
        act_dates_dic = {ds_add(reg_date, keep_day - 1): 'd%d_keep' %
                         keep_day for keep_day in keep_days}
        keep_df = (reg_act_df
        ...
        keep_df['ds'] = reg_date
        keep_df['dau'] = len(set(act_df.loc[act_df.act_ds == reg_date].user_id.tolist(
        )) | set(reg_df.loc[reg_df.reg_ds == reg_date].user_id.tolist()))
...
def keep_rate_date(date):
    ...
    act_dates_dic = {ds_add(date, keep_day - 1): 'd%d_keep' %
                     keep_day for keep_day in keep_days}
    ...
    keep_df = reg_act_df.loc[reg_act_df.reg_ds == reg_date, [
        'reg_user_num'] + act_dates_dic.keys()].rename(columns=act_dates_dic).sum().fillna(0).to_frame().T
    for c in act_dates_dic.values():
        keep_df[c + 'rate'] = keep_df[c] / keep_df.reg_user_num
    keep_df['ds'] = reg_date
...
if __name__ == '__main__':
    for platform in ['sanguo_ks','sanguo_tw','sanguo_tl','sanguo_tt',]:
        settings_dev.set_env(platform)
        for date in date_range('20170507', '20170508'):
            dis_keep_rate(date)
