...
def tmp_20170405_week_data(date):
    ...
    #消费
    spend_sql = '''
        select '{date}' as monday, sum(coin_num) as spend_coin from raw_spendlog where ds>='{date}' and ds<='{date_7}' and user_id not in {uids}
    '''.format(uids=uids, date=date, date_7=ds_add(date, 6))
    print spend_sql
    spend_df = hql_to_df(spend_sql)
    #下周消费
    spend_next_sql = '''
        select '{date}' as monday, sum(coin_num) as spend_next from raw_spendlog where ds>='{date_8}' and ds<='{date_14}' and user_id not in {uids}
    '''.format(uids=uids, date=date, date_8=ds_add(date, 7), date_14=ds_add(date, 13))
...
if __name__ == '__main__':
    ...
    result.to_excel(r'/home/kaiqigu/桌面/机甲无双-金山-审计-10月11月12月.xlsx', index=False)