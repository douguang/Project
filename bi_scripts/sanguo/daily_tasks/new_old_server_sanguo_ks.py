...
# -*- coding: UTF-8 -*-
'''
@Time    : 2017/8/28 0028 11:17
@Author  : Andy 
@File    : new_old_server_sanguo_ks.py
@Software: PyCharm
Description :
'''
...
def data_reduce():
    ...
    love_briage_sql = '''
        select ds,reverse(substring(reverse(user_id),8)) as server_id,count(distinct user_id) as num from parse_actionlog where ds='20170827' and a_typ like '%love%' group by ds,server_id
    '''
    print love_briage_sql
    love_briage_df = hql_to_df(love_briage_sql).fillna(0)
    print love_briage_df.head()
    result_2_df = love_briage_df.merge(is_new_old_server_df, on=['ds', 'server_id'], how='left')
    result_2_df = result_2_df.groupby(['ds', 'is_not_new', ]).agg({
        'num': lambda g: g.sum(),
    }).reset_index()


    result_df = result_df.merge(result_1_df, on=['ds', 'is_not_new'], how='left')
    result_df = result_df.merge(result_2_df, on=['ds', 'is_not_new'], how='left')
    print result_df.head()

    result_df.to_excel(r'E:\sanguo_ks_new_old_server-20170828-2.xlsx', index=False)
