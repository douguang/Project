...
import time
import numpy as np
from mart_sql_cfg import sanguo_mid_actionlog_sql, sanguo_raw_info_sql, sanguo_nginx_sql, paylog_sql, spendlog_sql, sanguo_mid_actionlog_2_sql


def mart_data_all(date):
    print 'mart_data_all>>>start', time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))

    df_1, df_2, df_3 = hqls_to_dfs([sanguo_raw_info_sql.format(date=date), sanguo_mid_actionlog_sql.format(
        date=date), paylog_sql.format(date=date)])
    df_2 = df_2.drop_duplicates(['user_id'])
    df_4, df_5 = hqls_to_dfs([spendlog_sql.format(
        date=date), sanguo_mid_actionlog_2_sql.format(date=date)])
    # df_6 = hql_to_df(sanguo_nginx_sql.format(date=date))
    # df_6 = df_6.drop_duplicates(['user_id'])
    ...
    print 'mart_data_all>>>end', time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
    return result_df
