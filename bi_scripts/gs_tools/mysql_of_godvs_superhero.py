...
def get_info(date):
    ...
    try:
        ...
        try:
            old_df.to_csv(
                r'/home/data/bi_scripts/gs_tools/backup_data/backup_common_vipinfo_%s'
                % backtime,
                index=False)
            print 'common_vip_info_csv_backup'
        except Exception, e:
            print e
            print 'common_vip_info_csv_backup_error'
        # 备份前一天数据到数据库
        try:
            back_table = 'backup_vip_info_common'
            back_del_sql = 'delete from {0} where ds="{1}"'.format(back_table,
                                                                   date)
            update_mysql(back_table, old_df, back_del_sql, 'godvs')
            print 'common_vip_info_mysql_backup'
        except Exception, e:
            print e
            print 'common_vip_info_mysql_backup_error'
            print 'run failed'
            exit()
        try:
            # 获取需求数据
