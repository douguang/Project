...
def get_info(date, platform):
    if platform == 'dancer_pub':
        platform_2 = 'dancer_ks'
    else:
        platform_2 = platform
    table = '%s_vipuserinfo'%platform_2
    # 获取前一天数据
    ...
    try:
        ...
        try:
            old_df.to_csv(
                r'/home/data/bi_scripts/gs_tools/backup_data/backup_dancer_vipinfo_%s'
                % backtime,
                index=False)
            print 'dancer_vip_info_csv_backup'
        except Exception, e:
            print e
            print 'dancer_vip_info_csv_backup_error'
        # 备份前一天数据到数据库
        try:
            back_table = 'backup_vip_info_dancer'
            back_del_sql = 'delete from {0} where ds="{1}"'.format(back_table,
                                                                   date)
            update_mysql(back_table, old_df, back_del_sql, 'godvs')
            print 'dancer_vip_info_mysql_backup'
        except Exception, e:
            print e
            print 'dancer_vip_info_mysql_backup_error'
            print 'run failed'
            exit()
        try:
            # 获取需求数据
...
if __name__ == '__main__':
    ...
    for platform in ['dancer_pub', 'dancer_mul']:
        get_info(date, platform)