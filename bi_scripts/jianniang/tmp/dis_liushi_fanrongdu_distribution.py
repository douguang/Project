...
@Time    : 17-3-20 下午2:58
@Author  : Andy 
@File    : dis_liushi_fanrongdu_distribution.py
@Software: PyCharm
Description :   次日流失玩家的繁荣度分布
'''
...
def dis_liucun_combat_distribution(date):
    ...
    ciliucun_sql = '''
          ...
          from (
            select ds,user_id from raw_info where ds='{date}' and regexp_replace(substring(reg_time,1,10),'-','')  = '{date}'  and user_id not in (select user_id from raw_info where ds='{tormo_date}' group by user_id)  group by ds,user_id
          )t1
          left outer join(
