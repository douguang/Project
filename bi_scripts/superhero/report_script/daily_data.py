...
if __name__ == '__main__':
    for platform in ['superhero_bi', 'superhero_vt',]:
        settings_dev.set_env(platform)
        # date = '20170301'
        for date in ['20171004','20171011']:
            print date
            date_ago = ds_add(date, -6)
            daily_df = get_daily_data(date, date_ago)
            week_df = week_daily_data(date, date_ago)
            daily_df.to_excel(
                '/Users/kaiqigu/Documents/report/{platform}_daily_df_{date}.xlsx'.format(date=date,platform=platform))
            week_df.to_excel(
                '/Users/kaiqigu/Documents/report/{platform}_daily_week_df_{date}.xlsx'.format(date=date,platform=platform))
