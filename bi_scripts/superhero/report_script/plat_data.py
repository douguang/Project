...
def get_act_df(now_df, last_df):
    ...
    df = get_rank(act_result, 'act_num', 9)[
        ['platform_2', 'last_act_num', 'act_num', 'rank']]
    other_df = (pd.DataFrame(
        {'platform_2': ['other'],
         'act_num': [act_df.sum().act_num - df.sum().act_num],
         'last_act_num':
         [last_act_df.sum().last_act_num - df.sum().last_act_num],
         'rank': [10]})[['platform_2', 'last_act_num', 'act_num', 'rank']])
...
if __name__ == '__main__':
    ...
    for date in ['20170531']:
        ...
        # 本周新增，上周新增
        now_reg_df = now_df[(now_df.reg_time >= date_ago)
                            & (now_df.reg_time <= date)]
        ...
        now_pay_df = pay_result[(pay_result.ds >= date_ago)
                                & (pay_result.ds <= date)]
        last_pay_df = pay_result[(pay_result.ds >= week_ago) & (
            pay_result.ds < date_ago)]
        income_result = get_pay_df(now_pay_df, last_pay_df)
        income_result = income_result.T

        act_result.to_excel(
            r'E:\Data\output\superhero\plat_act_result_{date}.xlsx'.format(date=date))
        reg_result.to_excel(
            r'E:\Data\output\superhero\plat_reg_result_{date}.xlsx'.format(date=date))
        income_result.to_excel(
            r'E:\Data\output\superhero\plat_income_result_{date}.xlsx'.format(date=date))
