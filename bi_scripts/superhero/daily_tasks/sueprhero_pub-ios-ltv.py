...
final_date = end_date
date = start_date
ltv_days = [3,7, 14, 30,60,90,]
print end_date
pub_ios_sql = '''
    select account,case when account like 'cmgeapp%' then 'ios' else 'pub' end as country from mid_info_all where ds='{end_date}' group by account,country
'''.format(end_date=end_date)
result = hql_to_df(pub_ios_sql)
print result.head()
...
reg_sql = '''
select regexp_replace(to_date(create_time),'-','') AS ds,'plat' as platform_2,account from raw_info WHERE ds >= '{0}'AND ds <= '{1}' group by ds,platform_2,account
...
result_df.to_excel('/Users/kaiqigu/Documents/Superhero/superhero_pub_90ltv.xlsx')
print 'end'
