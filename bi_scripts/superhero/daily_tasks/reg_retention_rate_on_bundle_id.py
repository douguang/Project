...
@site:  
@software: PyCharm 
@file: reg_retention_rate_on_bundle_id.py 
@time: 17/12/13 下午5:09 
"""
...
def data_reduce(start_date,end_date,final_date):
    ...
    result_sql = '''
            select uid,bundle_id as country from mid_info_all where ds='{final_date}' group by uid,bundle_id
        '''.format(final_date=final_date, )
    print result_sql
    result = hql_to_df(result_sql)
    result = result.drop_duplicates('uid')
...
if __name__ == '__main__':
    ...
    res.to_excel('/Users/kaiqigu/Documents/Superhero/%s-分包留存_20171213.xlsx' % platform)
