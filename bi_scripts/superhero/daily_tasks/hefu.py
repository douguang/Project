...
def get_ser_data():
    ...
    file_in = path +  'superhero_vt_list1.txt'
    file_out = path + 'superhero_vt_res2.txt'
    f_in = open(file_in,'r')
...
if __name__ == '__main__':
    ...
    # info_result_df = info_ser.groupby(['father_ser', ]).agg({
    #     'uid': lambda g: g.nunique(),
    # }).reset_index()
    # info_result_df.to_excel('/Users/kaiqigu/Documents/Superhero/超级英雄-国服-father_ser数据2—{plat}_{date}.xlsx'.format(plat=plat, date=date))
    ...
    result_df.to_excel('/Users/kaiqigu/Documents/Superhero/超级英雄-越南-合服数据2—{plat}_{date}.xlsx'.format(plat=plat,date=date))
...
# =======================下面为未合服的数据==========================
    ...
    if len(info_son_ser) > 0:
        ...
        # info_result_df = info_son_ser.groupby(['son_ser',]).agg({
        #     'uid': lambda g: g.nunique(),
        # }).reset_index()


        # info_result_df.to_excel('/Users/kaiqigu/Documents/Superhero/超级英雄-国服-son_ser数据2—{plat}_{date}.xlsx'.format(plat=plat, date=date))
        info_result_df['avg_dau'] = info_result_df['uid']*1.0/7
        del info_result_df['uid']
        # 近7天充值总人数
        ...
        result_df.to_excel('/Users/kaiqigu/Documents/Superhero/超级英雄-越南-未合服数据2—{plat}_{date}.xlsx'.format(plat=plat,date=date))
    else:
