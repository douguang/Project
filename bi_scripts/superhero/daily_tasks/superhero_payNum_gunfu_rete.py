...
@site:  
@software: PyCharm 
@file: superhero_payNum_gunfu_rete.py 
@time: 17/9/1 下午5:12 
"""
...
def data_reduce():
    ...
    ds_act_sql = '''
        select ds,reverse(substring(reverse(uid),8)) as server,uid,sum(order_money) as order_money from raw_paylog where ds>='20170101' and platform_2 != 'admin' and platform_2 != 'admin_test'group by ds,server,uid
    '''
    ...
    ds_act_num_df = ds_act_num_df.groupby(['ds', 'server','is_not_duokai']).agg({'uid': lambda g: g.nunique(),'order_money':lambda g: g.sum()}).reset_index()
    # ds_act_num_df = (ds_act_num_df.pivot_table('pay_num', ['plat', 'vip'],
    ...
    result.to_excel('/Users/kaiqigu/Documents/Sanguo/超级英雄-BI-充值及滚服数据_20170901.xlsx')
