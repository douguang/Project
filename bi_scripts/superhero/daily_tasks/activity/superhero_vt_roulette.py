...
def data_reduce(date):
    dau_sql = '''
        select vip_level as vip,count(distinct uid) as dau from raw_info where ds='{date}' group by vip
    '''.format(date=date,)
    dau_df = hql_to_df(dau_sql)
    print dau_df.head(3)

    canyu_sql = '''
        select pre_vip as vip,count(distinct uid) as canyuNum from raw_action_log where ds='{date}' and action like '%roulette%'  and action not  like '%index%' group by vip
    '''.format(date=date,)
