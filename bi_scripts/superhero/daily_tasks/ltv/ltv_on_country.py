...
# -*- coding: UTF-8 -*-
'''
@Time    : 2018/2/24 0024 19:06
@Author  : Andy 
@File    : ltv_on_country.py
@Software: PyCharm
Description :
'''
...
def data_reduce(start_date,end_date,final_date):
    ...
    result_sql = '''
        select account,ip as country from mid_info_all where ds='{final_date}' group by account,country
    '''.format(final_date=final_date,)
    result = hql_to_df(result_sql)
    result = result.drop_duplicates('account')

    result['country'] = result['country'].astype(basestring)
...
if __name__ == '__main__':
    ...
    res.to_excel('E:\%s-countryLTV_20180224.xlsx' % platform)