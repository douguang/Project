...
def data_reduce(start_date,end_date,final_date):
    date = start_date
    ltv_days = [1,2,3,4,5,6,7, 14, 30,45,60,90,180]

    # 读取IP地址'
    res_list = []
    res_text = pd.read_csv(r'E:\q_new_rate.txt')
    ...
    for date in date_range(start_date, end_date):
        ...
        col_df = (reg_data.groupby(['platform_2', 'country']).count().account.reset_index().rename(columns={'account': 'reg_num'}))
        col_df['ds'] = date
        for ltv_day in ltv_days:
            ...
            if ltv_end_date > final_date:
                ...
                col_df['reg_num'] = col_df['reg_num'].astype(int)
                col_df['reg_num'] = col_df.replace({0:1}).reg_num
...
if __name__ == '__main__':
    ...
    res.to_excel(r'E:\%s-4-5-6monthLTV_20171227-2.xlsx' % platform)
