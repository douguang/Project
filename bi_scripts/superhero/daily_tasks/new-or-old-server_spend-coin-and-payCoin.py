...
@site:  
@software: PyCharm 
@file: new-or-old-server_spend-coin-and-payCoin.py 
@time: 17/9/20 下午4:56 
"""
...
new_or_old_df = new_or_old_df.dropna()
...
# 充值人数-充值金额
pay_sql = '''
    select ds,uid,reverse(substring(reverse(uid), 8)) AS server,sum(order_coin) as order_money from raw_paylog where ds>='20170713'  and platform_2 !='admin_test' group by ds,uid,server
'''
...
# 付费
is_gun_pay_sql = '''
    select ds,uid,reverse(substring(reverse(uid),8))as server,sum(order_coin) as order_money from raw_paylog where ds>='20170713' and platform_2 != 'admin_test' 
...
result.to_excel('/Users/kaiqigu/Documents/Superhero/超级英雄-越南-充值对比-总_20170919(充值钻石).xlsx')
