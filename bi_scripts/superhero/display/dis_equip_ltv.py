...
def dis_equip_ltv(date):
    ...
    # 注册数据
    reg_sql = '''
    SELECT account, 'plat' as plat,platform, t1.ds
    FROM (SELECT account, substr(uid, 1, 1) AS plat, row_number() OVER (PARTITION BY account ORDER BY create_time ASC) AS rn, bundle_id AS platform, regexp_replace(to_date(create_time), '-', '') AS ds
        FROM mid_info_all
        WHERE ds = '{date}'
            AND to_date(create_time) >= '{f_start_date}'
            AND to_date(create_time) <= '{f_date}'
        ) t1
    WHERE t1.rn = 1 and platform != ''
    '''.format(date=date,
        ...
        if date_to_update >= server_start_date:
            ...
            for ltv_ds in ltv_days:
                ...
                if ltv_end_date <= date:
                    ...
                    if ltv_dfs.__len__() != 0 or result_ltv_df.__len__() != 0:
    ...
    if dfs.__len__() != 0:
        ...
        if settings_dev.code == 'superhero_bi':
            update_mysql(table, final_result[column], del_sql, 'superhero_pub')
        else:
...
if __name__ == '__main__':
    for platform in ['superhero_bi']:
        ...
        for date in date_range('20171124', '20180108'):
            dis_equip_ltv(date)
