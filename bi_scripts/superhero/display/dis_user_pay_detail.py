...
def dis_user_pay_detail(date):
    ...
    '''.format(date=date)
    # reg_sql = '''
    #     select regexp_replace(min(to_date(create_time)),'-','') as ds,substr(platform_2,1,1) as  plat,account from mid_info_all where ds='{date}' and regexp_replace(to_date(create_time),'-','') ='{date}' group by plat,account
    #     '''.format(date=date)
    assist_sql = '''
    SELECT account,
           plat,
    ...
    result_df['old_pay_num'] = result_df['pay_num'] - result_df[
        'new_day_pay_num']
...
if __name__ == '__main__':
    ...
    for date in date_range('20180317', '20180330'):
        print date
