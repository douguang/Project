...
def dis_keep_rate(excute_date):
    ...
    act_df['act'] = 1
    reg_act_df = (
        act_df.pivot_table('act', ['user_id', 'platform_2', 'plat'], 'ds')
    ...
    dau_result = dau_df.groupby(
        ['ds', 'plat', 'platform_2']).count().reset_index().rename(columns={
            'user_id': 'dau',
        })
    ...
    for reg_date in reg_dates:
        ...
        keep_df = (reg_act_df.loc[reg_act_df.ds == reg_date, [
            ...
            ['platform_2', 'plat']).sum().reset_index().fillna(0))
        result = dau_result.loc[dau_result.ds == reg_date].merge(
            keep_df, on=['plat', 'platform_2'],
            how='outer').fillna(0)
