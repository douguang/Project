...
def dis_country_ltv(date):
    ...
    FROM (SELECT account, substr(uid, 1, 1) AS plat, row_number() OVER (PARTITION BY account ORDER BY create_time ASC) AS rn, ip AS platform, regexp_replace(to_date(create_time), '-', '') AS ds
        FROM mid_info_all
        WHERE ds = '{date}'
    ...
    reg_df['platform'] = reg_df['platform'].astype(basestring)
    ...
    def ip_lines():
        for _, row in reg_df.iterrows():
            ip = row.platform
            try:
                platform = IP.find(ip).strip().encode("utf8")
                if '中国台湾' in platform:
                    platform = '台湾'
                elif '中国香港' in platform:
                    platform = '香港'
                elif '中国澳门' in platform:
                    platform = '澳门'
                elif '中国' in platform:
                    platform = '中国'
                yield [row.account,row.plat,platform,row.ds,]
            except:
                platform = 'None'
                yield [row.account,row.plat,platform,row.ds,]

    reg_df = pd.DataFrame(ip_lines(), columns=['account', 'plat','platform','ds']).drop_duplicates()
    print reg_df.head()
...
if __name__ == '__main__':
    for platform in ['superhero_mul']:
        ...
        result = dis_country_ltv('20180209')
