...
def dis_card(date):
    ...
    WHERE ds ='{date}'
    '''.format(date=date)
    card_df, super_df, gs_df = hqls_to_dfs(
        [card_sql, super_sql.format(date=date), gs_sql])

    total_dic = {'zhuansheng': 'total_num',
                 'user_id': 'have_user_num', 'is_fight': 'attend_num'}
    # 卡牌总张数、拥有人数、上阵人数
    total_df = (card_df.groupby(['card_id', 'card_name', 'plat']).agg({
        'zhuansheng': 'count',
        'user_id': 'nunique',
        'is_fight': 'sum',
    ...
    # 转生
    zhuangsheng_df = (card_df.groupby(['card_id', 'plat', 'zhuansheng']).count().user_id
                      .reset_index().pivot_table('user_id', ['card_id', 'plat'], 'zhuansheng')
                      .reset_index().fillna(0).rename(columns=zhuangsheng_dic))
    # 补充不存在的转生数据
    for i in ['d%d_num' % num for num in zhuangsheng_num]:
        if i not in zhuangsheng_df.columns:
            zhuangsheng_df[i] = 0

    # 综合卡牌转生数据
    zhuansheng_result_df = total_df.merge(
        zhuangsheng_df, on=['card_id', 'plat'], how='left').fillna(0)

    # ======================================================================
    jinjie_num = [i for i in range(0, 61)]
    jinjie_dic = {num: 'd%d_evo_num' % num for num in jinjie_num}
    # 进阶
    jinjie_df = (card_df.groupby(['card_id', 'plat', 'jinjie']).count().user_id
                 .reset_index().pivot_table('user_id', ['card_id', 'plat'], 'jinjie')
    ...
    for i in ['d%d_evo_num' % num for num in jinjie_num]:
        if i not in jinjie_df.columns:
            jinjie_df[i] = 0
    # 综合卡牌进阶数据
    jinjie_result_df = total_df.merge(
        jinjie_df, on=['card_id', 'plat'], how='left').fillna(0)

    # ======================================================================
    super_num = [i for i in range(0, 9)]
    super_dic = {num: 'd%d_super_num' % num for num in super_num}
    # 排除卡牌超进化的测试用户
    super_df = super_df[~super_df['user_id'].isin(
        gs_df.user_id.values)]
    # # 保留神卡
    # super_df = super_df[super_df['card_id'].isin(
    #     total_df.card_id.values)]
    # 超进化
    super_result = (super_df.groupby(['card_id', 'plat', 'super_step_level']).count().user_id
                    .reset_index().pivot_table('user_id', ['card_id', 'plat'], 'super_step_level')
    ...
    # 综合卡牌超进化数据
    super_result_df = total_df.merge(
        super_result, on=['card_id', 'plat'], how='left').fillna(0)
    ...
    zhuansheng_column = ['ds', 'card_id', 'card_name', 'have_user_num', 'attend_num', 'use_rate', 'd0_relive_num',
                         'd1_relive_num', 'd2_relive_num', 'd3_relive_num', 'd4_relive_num', 'd5_relive_num', 'd6_relive_num', 'd7_relive_num']
    jinjie_column = ['ds', 'card_id', u'card_name', 'have_user_num',
                     'attend_num', 'use_rate'] + ['d%d_evo_num' % num for num in jinjie_num]
    super_column = ['ds', 'card_id', u'card_name', 'have_user_num', 'attend_num', 'use_rate'
                    ] + ['d%d_super_num' % num for num in super_num]

    update_data(zhuansheng_result_df, zhuansheng_table,
                zhuansheng_column, date)
