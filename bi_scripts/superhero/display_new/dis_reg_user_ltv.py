...
# Ltv日期
ltv_days = [1, 3, 7, 14, 30, 60]


def dis_sup_act_ltv_ondate(date, exec_date):
    act_date = DateFormat(date)
    ...
    for plat in new_act_df.drop_duplicates('plat').plat.tolist():
        ...
        for i in ltv_date_list:
            data = plat_pay_df.loc[(plat_pay_df['ds'] >= date) & (
                plat_pay_df['ds'] <= i)]
    ...
    column = ['ds', 'reg_user_num'] + \
        ['d%d_pay_num' % day for day in ltv_days[1:]] + \
        ['d%d_ltv' % day for day in ltv_days[1:]]
