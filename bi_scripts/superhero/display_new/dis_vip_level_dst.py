...
def dis_vip_level_dst(date):
    ...
    total_df = assist_df.groupby(['plat', 'vip_level']).sum(
    ).reset_index().rename(columns=total_dic)[total_columns]
    # 新增VIP用户数据
    new_columns = ['plat', 'vip_level', 'new_vip_user', 'new_vip_pay_coin']
    new_dic = {'user_num': 'new_vip_user', 'sum_coin': 'new_vip_pay_coin'}
    new_df = assist_df[assist_df.is_new_pay == 1].rename(columns=new_dic)[
        new_columns]
    # 老VIP用户数据
    old_columns = ['plat', 'vip_level', 'old_vip_user', 'old_vip_pay_coin']
    old_dic = {'user_num': 'old_vip_user', 'sum_coin': 'old_vip_pay_coin'}
    old_df = assist_df[assist_df.is_new_pay == 0].rename(columns=old_dic)[
        old_columns]
    # 数据汇总
    result_df = (total_df
                 .merge(new_df, on=['plat', 'vip_level'], how='left')
                 .merge(old_df, on=['plat', 'vip_level'], how='left')
                 .fillna(0))
    # 新增vip登陆占比
    ...
    column = ['ds', 'vip_level', 'income', 'new_vip_user', 'old_vip_user', 'vip_user_total',
              'new_vip_pay_coin', 'old_vip_pay_coin', 'new_vip_login_rate', 'new_vip_pay_rate']
