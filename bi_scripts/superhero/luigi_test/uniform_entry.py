...
class SuperheroDaily(sl.WorkflowTask):
    '''
    执行入口
    '''
    date = luigi.DateParameter(
...
class SuperheroDailyRaw(sl.WorkflowTask):
    '''
    执行入口
    '''
    date = luigi.DateParameter(
...
# 临时用，之后补Mart数据
class SuperheroDailyMart(sl.WorkflowTask):
    '''
    执行入口
    '''
    date = luigi.DateParameter(
...
# 临时用，之后补MidInfoAll数据
class SuperheroDailyMidInfoAll(sl.WorkflowTask):
    '''
    执行入口
    '''
    date = luigi.DateParameter(
...
# # 用于补特定的任务
# class SuperheroDailyDemo(sl.WorkflowTask):
...
#                                     str(self.date))
#         for job_deps in settings_dev.job_deps:
#             if job_deps not in os.listdir(history_path) and job_deps == 'parse_nginx':
#                 tasks.append(self.new_task('SuperheroDailyDemo',
#                                            SuperheroDailyDemo,
#                                            platform=self.platform,
#                                            job=job_deps,
#                                            date=self.date))
