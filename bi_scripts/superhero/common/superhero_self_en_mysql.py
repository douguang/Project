...
table_name = [
'dis_reg_user_ltv', # 注册用户LTV(account)
'dis_gift_pay_detail', # 礼包购买详情
'dis_day_coin_spend', # 每日钻石消费
'dis_daily_data', # 日常数据
'dis_card_evo', # 卡牌进阶
'dis_card_relive', # 卡牌转生
'dis_card_use_rate', # 卡牌使用率
'dis_keep_rate', # 留存率
'dis_spend_detail', # 消费详情
'dis_spend_detail_coin_shop', #钻石商城
'dis_spend_detail_quota_shop',# 限购商城
'dis_card_equip_quality', # 命运装备品质
'dis_card_equip_num' # 命运装备数量
]
...
def database_table():
    # 查询数据库中的所有表
    for plat in ['superhero_pub','superhero_ios','superhero_qiku','superhero_vt']:
