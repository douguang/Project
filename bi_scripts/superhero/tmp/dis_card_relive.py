...
if __name__ == '__main__':
    ...
    date = '20160801'
    plat = None
# def dis_card_relive_one(date, plat=None):
    plat = plat or settings.platform

    zhuangsheng_num = [0, 1, 2, 3, 4, 5, 6, 7]
    zhuangsheng_dic = {num: 'd%d_num' % num for num in zhuangsheng_num}
    card_sql = '''
    SELECT uid,
    ...
    # 转生
    card_result_df['zhuansheng_num'] = 1
    zhuangsheng_df = (card_result_df.groupby(['card_id','zhuansheng']).count().reset_index()
        .loc[:,['card_id','zhuansheng','zhuansheng_num']]
        .pivot_table(
            'zhuansheng_num', ['card_id'],
            'zhuansheng').reset_index()
        .fillna(0).rename(columns=zhuangsheng_dic))
    ...
    # 使用率
    result_df = (have_num_df.merge(fight_num_df,
                                   on=['card_id'],
                                   how='outer').merge(
                                       zhuangsheng_df,
                                       on=['card_id'],
    ...
    columns = ['ds', 'card_id', u'card_name',
               'have_num', 'fight_num', 'have_rate'
               ] + ['d%d_num' % num for num in zhuangsheng_num]
    result_df = result_df[columns]
    rename_dic = {'name': 'card_name',
                  'have_num': 'have_user_num',
                  'fight_num': 'attend_num',
                  'have_rate': 'use_rate'}
    rename_dic2 = {'d%d_num' % num: 'd%d_relive_num' % num
                   for num in zhuangsheng_num}
    result_df = result_df.rename(columns=rename_dic).rename(
        columns=rename_dic2)
    print result_df
    result_df.to_excel('/Users/kaiqigu/Downloads/Excel/dis_card_relive.xlsx')

#     # 更新MySQL表
#     table = 'dis_card_relive'
#     del_sql = 'delete from {0} where ds="{1}"'.format(table, date)
#     update_mysql(table, result_df, del_sql, plat)


# def dis_card_relive(date):
#     if settings.code_dir == 'superhero_bi':
#         for plat in ['superhero_pub', 'superhero_ios']:
#             print plat
#             dis_card_relive_one(date, plat)
#     else:
#         dis_card_relive_one(date)


# if __name__ == '__main__':
#     settings.set_env('superhero_vt')
#     date = '20160617'
#     dis_card_relive(date)
