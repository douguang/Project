...
def jinjie_info(final_result,date,num1,num2):
    df_name = final_result[final_result['jinjie'] >= num1]
    df_name = df_name[df_name['jinjie'] <= num2]
    num_name = df_name.groupby('card_id').count().reset_index()
    num_name['r%s%s_num' %(num1,num2)] = num_name['uid']
    columns = ['card_id','r%s%s_num' %(num1,num2)]
...
def zhuansehng_info(final_result,date):
    for i in range(23,61):
        df_name = final_result[final_result['jinjie']== i]
        num_name = df_name.groupby('card_id').count().reset_index()
    ...
    df_name = final_result[final_result['jinjie']== 0]
    num_name = df_name.groupby('card_id').count().reset_index()
    num_name['r0_num'] = num_name['uid']
    columns = ['card_id','r0_num']
    num_name = num_name[columns]

    df_name_0004 = jinjie_info(final_result,date,0,4)
    df_name_0523 = jinjie_info(final_result,date,5,23)
    df_name_2440 = jinjie_info(final_result,date,24,40)
    df_name_4150 = jinjie_info(final_result,date,41,50)
    df_name_5160 = jinjie_info(final_result,date,51,60)
    zhuansheng_num =  (zhuansheng_num
                                    .merge(num_name,on=['card_id'],how='outer')
                                    .merge(df_name_0004,on=['card_id'],how='outer')
                                    .merge(df_name_0523,on=['card_id'],how='outer')
                                    .merge(df_name_2440,on=['card_id'],how='outer')
                                    .merge(df_name_4150,on=['card_id'],how='outer')
                                    .merge(df_name_5160,on=['card_id'],how='outer')
                      )
...
if __name__ == '__main__':
    ...
    columns = ['ds','card_id','name','have_num','fight_num','r04_num','r523_num','r2440_num', 'r4150_num'
    ,'r5160_num','r0_num','r23_num','r24_num','r25_num','r26_num','r27_num','r28_num','r29_num','r30_num'
    ,'r31_num','r32_num','r33_num','r34_num','r35_num','r36_num','r37_num','r38_num','r39_num','r40_num'
    ,'r41_num','r42_num','r43_num','r44_num','r45_num','r46_num','r47_num','r48_num','r49_num','r50_num'
    ,'r50_num','r52_num','r53_num','r54_num','r55_num','r56_num','r57_num','r58_num','r59_num','r60_num']
    final_result = final_result[columns]
    final_result = final_result.fillna(0)
    final_result.to_excel('/Users/kaiqigu/Downloads/Excel/jinjie.xlsx')
