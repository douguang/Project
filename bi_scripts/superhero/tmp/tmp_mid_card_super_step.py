...
Description : 越南参与神域玩家卡牌培养情况--超进化
...
def zhuansehng_info(final_result,date):
    for i in range(0,9):
        df_name = final_result[final_result['super_step_level']== i]
...
def final_result_data_info(final_set_field_df,date):
    ...
    super_sql = "select uid,card_id,super_step_level from raw_super_step where ds = '{0}'".format(date)
    print super_sql
    super_df = hql_to_df(super_sql)
    super_df['card_id'] = super_df['card_id'].map(lambda s:str(s))
    super_df['uid_card'] = super_df['uid'] + super_df['card_id']
    super_df['is_final_result'] = super_df['uid_card'].isin(result_data.uid_card.values)
    result_data = super_df[super_df['is_final_result'] == True]
...
if __name__ == '__main__':
    ...
    super_df['card_id'] = super_df['card_id'].map(lambda s:str(s))
    super_df['uid_card'] = super_df['uid'] + super_df['card_id']
    super_df['is_final_result'] = super_df['uid_card'].isin(final_result.uid_card.values)
    final_result = super_df[super_df['is_final_result'] == True]
    ...
    columns = ['ds','card_id','name','have_num','fight_num','r0_num','r1_num','r2_num', 'r3_num','r4_num','r5_num','r6_num','r7_num','r8_num']
    final_result = final_result[columns]
    final_result = final_result.fillna(0)
    final_result.to_excel('/Users/kaiqigu/Downloads/Excel/super_step.xlsx')
