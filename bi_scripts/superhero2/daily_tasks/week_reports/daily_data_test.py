...
def get_daily_data(date, date_ago):
    ...
    FROM
      (SELECT ds,
       ...
       GROUP BY ds) act
    JOIN
      (select ds, count (distinct user_id)  reg_num
from  parse_info
where ds  >='{date_ago}'
AND ds <='{date}'
...
def week_daily_data(date, date_ago):
    ...
    FROM
      (SELECT '{date}' AS ds,
       ...
       GROUP BY '{date}') act
    JOIN
      (select  '{date}' AS ds,  count (distinct user_id)  reg_num
from  parse_info
where ds  >='{date_ago}'
AND ds <='{date}'
and  regexp_replace(to_date(reg_time), "-", "")>='{date_ago}'
and  regexp_replace(to_date(reg_time), "-", "")<='{date}') reg ON act.ds = reg.ds
    JOIN
      (SELECT '{date}' AS ds,
              count(DISTINCT user_id) pay_num,
              sum(order_money) sum_money
...
if __name__ == '__main__':
    ...
    for plat in plat_list:
        ...
        for date in ['20180801']:
            ...
            daily_df.to_excel(
                r'C:\Users\Administrator\Desktop\0803\{plat}_daily_df_{date}.xlsx'.format(date=date,plat=plat))
            week_df.to_excel(
                r'C:\Users\Administrator\Desktop\0803\{plat}_daily_week_df_{date}.xlsx'.format(date=date,plat=plat))
