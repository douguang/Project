...
def dis_coin_save(date):
    ...
    result_df = coin_df.groupby(['coin_num', pd.cut(coin_df.level, ranges)]).count().user_id.fillna(0).reset_index()
    print result_df
    result_df = pd.pivot_table(result_df, values='user_id', index=['coin_num'], columns='level', aggfunc='sum').fillna(0).rename(columns=columns_to_rename).reset_index()
    result_df['ds'] = date
    print result_df
