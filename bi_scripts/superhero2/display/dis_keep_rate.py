...
def dis_keep_rate(date):
    ...
    #     print d
    # print mid_df
    dau_df = dau_df.merge(mid_df, on='ds', how='left').reset_index()
    dau_df['reg_user_num'] = dau_df['d1_user']
    for i in keep_days:
        d = str(i)
        dau_df['d' + d + '_keeprate'] = dau_df['d' + d + '_user'] / dau_df['reg_user_num']
    columns = ['ds', 'reg_user_num', 'dau'] + ['d%d_keeprate'%d for d in keep_days]
    dau_df = dau_df[columns].drop('d1_keeprate',axis=1)
