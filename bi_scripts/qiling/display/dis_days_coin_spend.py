...
def dis_days_coin_spend(date):
    ...
    if name_1 != 'ds':
        result = pd.DataFrame(
            columns=['ds', 'server', 'vip', 'new_coin', 'free_get_coin', 'pay_get_coin', 'coin_save', 'coin_spend'])
        print result.head(2)
...
if __name__ == '__main__':
    ...
    for platform in ['qiling_ios',]:
        settings_dev.set_env(platform)
        for date in date_range('20170203', '20170204'):
            dis_days_coin_spend(date)
    print "end"
