...
def dis_spirit_info(date):
    ...
    spirit_grouped_df['lv_counter'] = spirit_grouped_df['lv'].map(
        lambda x: Counter(x))
    spirit_grouped_df['spirit_num'] = spirit_grouped_df[
        'lv'].map(lambda x: len(x))
    ...
    # 更新MySQL表
    table_1 = 'dis_spirit_info'
    del_sql = 'delete from {0} where ds="{1}"'.format(table_1, date)
    update_mysql(table_1, spirit_grouped_df[columns], del_sql)
    ...
    buy_spirit_df['buy_spirit_times'] = buy_spirit_df.a_tar.map(
        lambda s: int(eval(s).get('buy_amount', 0)))
    buy_spirit_df['consume_money'] = - \
        (buy_spirit_df.freemoney_diff + buy_spirit_df.money_diff)
    buy_spirit_df['server'] = buy_spirit_df.user_id.map(lambda s: s[:-7])
    buy_spirit_vip_df = buy_spirit_df.merge(
        spirit_info_df[['user_id', 'vip']], on='user_id')
    ...
    if buy_spirit_result_df.empty is False:
        # 近三天进入真国战的uid
        uids_entered_cw_sql = '''
        SELECT ds,
               user_id
        ...
        uids_cw_vip_df = uids_entered_cw_df.merge(
            spirit_info_df[['user_id', 'vip']], on='user_id')
        uids_cw_vip_df['server'] = uids_cw_vip_df.user_id.map(lambda s: s[:-7])
        if uids_cw_vip_df.empty is False:
            # 三日活跃并进入真国战人数
            ...
            print cw_3d_act_df.head()
            # 当日活跃并进入真国战人数
            cw_today_act_df0 = uids_cw_vip_df.loc[uids_cw_vip_df.ds == date]
            if cw_today_act_df0.empty is False:
                cw_today_act_df = cw_today_act_df0.groupby(['server', 'vip']).user_id.nunique().reset_index().rename(
                    columns={'user_id': 'cw_today_act'})
                # 整合活跃且进入真国战人数
                ...
                spirit_buy_info_df['cw_today_act'] = 0
                spirit_buy_info_df['ds'] = date

    else:
        spirit_buy_info_df = pd.DataFrame({
            "server": "0",
            "vip": "0",
            "buy_user_num": "0",
            "consume_money": "0",
            "buy_spirit_times": "0",
            "cw_3d_act": "0",
            "cw_today_act": "0"}, index=["0"])
        spirit_buy_info_df['ds'] = date
    # 更新MySQL
    table_2 = 'dis_spirit_buy_info'
    del_sql = 'delete from {0} where ds="{1}"'.format(table_2, date)
    update_mysql(table_2, spirit_buy_info_df, del_sql)

if __name__ == '__main__':
    settings_dev.set_env('qiling_tx')
    dis_spirit_info('20170216')
