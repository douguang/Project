...
def dis_first_month_ltv(date):
    ...
    pay_daily_df = pay_df.pivot_table(
        'pay_rmb', ['user_id'], 'ds').reset_index().fillna(0)

    def yield_result():
        for server, server_open_date in servers_to_update.iteritems():
            server_uids_pay = reg_df[reg_df.server == server][
                ['user_id']].merge(pay_daily_df, how='left').fillna(0)
            reg_num = server_uids_pay.user_id.count()
            for week in ltv_weeks:
                date_to_sum = list(date_range(
                    server_open_date, ds_add(server_open_date, week * 7 - 1)))
                server_uids_pay['week%d_pay' %
                                week] = server_uids_pay[date_to_sum].sum(axis=1)
            for limit in recharge_limits:
                ...
                for week in ltv_weeks:
                    ...
                    if limit == 0:
                        ...
                        weekly_ltv = server_uids_pay[
                            server_uids_pay[c] < week * limit][c].mean()
    ...
    result_df = pd.DataFrame(yield_result(), columns=[
                             'server', 'server_open_date', 'reg_num', 'small_r_limit'] + ['week%d_pay' % week for week in ltv_weeks])
    #print result_df

    # 更新MySQL表

    del_sql = 'delete from {0} where server in {1}'.format(
        table, format_dates(servers_to_update))
    print del_sql
