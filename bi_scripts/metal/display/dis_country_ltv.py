...
def dis_platform_ltv(date):
    ...
    # 注册数据
    reg_ip_sql = '''
    select to_date(reg_time) as ds, account, platform as country
    from mid_info_all
    where ds = '{date}'
    ...
    for date_to_update in dates_to_update:
        ...
        if date_to_update >= server_start_date:
            ...
            ltv_dfs = use_reg_df.groupby(['ds', 'country']).agg({'account': lambda g: g.nunique()}).reset_index().rename(columns={
...
if __name__ == '__main__':
    for platform in ['metal_test']:
        ...
        # date = '20161120'
        for date in date_range('20161101', '20161130'):
