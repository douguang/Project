...
def dis_vip_level_dst(date):
    ...
    SELECT '{date}' AS ds,
           vip_level,
           count(t1.uid) AS new_vip_user,
           sum(pay) AS new_vip_pay_coin,
           count(DISTINCT t2.uid) AS new_pay_user
    FROM
      (SELECT uid,
              vip AS vip_level
       FROM parse_info
    ...
    SELECT '{date}' AS ds,
           vip_level,
           sum(pay) AS income,
           count(t1.uid) AS vip_user_total,
           count(t2.uid) AS pay_user_total
    FROM
      (SELECT uid,
              vip AS vip_level
       FROM parse_info
    ...
    silence_sql = '''
        with a as
        (select uid, vip as vip_level from mid_info_all where ds='{date}' and vip > 0 and regexp_replace(to_Date(act_time), '-', '') >= '{date_6}'),
        b as (select uid, max(ds) as pay_date from raw_paylog where ds <= '{date}' group by uid)
        select '{date}' AS ds, count(*) as silence_num, vip_level from a, b where a.uid = b.uid and b.pay_date>='{date_29}' and b.pay_date<'{date_6}' group by vip_level
    '''.format(date=date, date_6=ds_add(date, -6), date_29=ds_add(date, -29))
    # print silence_sql
    loss_sql = '''
        select '{date}' AS ds, count(distinct uid) as lost_num, vip as vip_level from mid_info_all where ds='{date}' and regexp_replace(to_Date(act_time), '-', '') >= '{date_13}'
