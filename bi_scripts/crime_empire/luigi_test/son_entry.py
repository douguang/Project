...
from lib.utils import error_log, DateFormat, Parse_data, ColorPrint, Parse_data_dict
from lib.utils import check_exist
from mid_hive_table import mid_dic
from luigi_tools import parse_actionlog, parse_nginx, parse_info, parse_card, parse_alliance, parse_skill, parse_city, parse_item, parse_daily_data, parse_nginx, parse_nginx_1
import pandas as pd
    ...
    def run(self):
        ...
        if os.path.exists(raw_path):
            ...
            if not os.path.exists(file_out):
                ...
                elif self.task_name == 'parse_daily_data':
                    Parse_data(file_in, file_out, parse_daily_data)
                elif self.task_name == 'parse_card':
                    Parse_data_dict(file_in, file_out, parse_card)
                elif self.task_name == 'parse_alliance':
                    Parse_data(file_in, file_out, parse_alliance)
                elif self.task_name == 'parse_item':
                    Parse_data_dict(file_in, file_out, parse_item)
                elif self.task_name == 'parse_city':
                    Parse_data_dict(file_in, file_out, parse_city)
                elif self.task_name == 'parse_skill':
                    Parse_data_dict(file_in, file_out, parse_skill)
                elif self.task_name == 'parse_nginx':
                    Parse_data(file_in, file_out, parse_nginx)
                #新增
                elif self.task_name == 'parse_nginx_1':
